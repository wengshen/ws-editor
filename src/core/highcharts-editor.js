...
var ws = {
      ...
      if (r.readyState === 4 && r.status === 200) {
        if (props.dataType === 'json') {
          try {
            ...
            if (ws.isFn(props.success)) {
              props.success(json);
            }
            events.emit('OK', json);
          } catch (e) {
            console.log('parse error', e);
            if (ws.isFn(props.error)) {
  ...
  arrToObj: function(arr) {
    ...
    if ((!ws.isArr(arr) && !ws.isBasic(arr)) || arr === false) {
      return arr;
    }
  ...
  setAttr: function(obj, path, value, index) {
    ...
    if (ws.isArr(obj)) {
      obj.forEach(function(thing) {
        ws.setAttr(thing, path, value, index);
      });
      return;
    }
    ...
    path.forEach(function(p, i) {
      if (i === path.length - 1) {
        ...
        if (typeof current[p] === 'undefined') {
          ...
          if (ws.isArr(current) && index >= 0 && index < current.length) {
            current = current[index];
          }
        }
      }
  ...
  isEmptyObjectArray: function(arr) {
    return ws.isObj(arr[0]) && arr.some(function(b) {
  ...
  merge: function(a, b, ignoreEmpty, excludeMap) {
    ...
    Object.keys(b).forEach(function(bk) {
      ...
      } else if (ws.isNull(b[bk]) || ws.isBasic(b[bk])) {
        a[bk] = b[bk];
      } else if (ws.isArr(b[bk])) {

        if (ws.isEmptyObjectArray(b[bk])) return;
        a[bk] = [];

        b[bk].forEach(function(i) {
          if (ws.isNull(i) || ws.isBasic(i)) {
            a[bk].push(i);
          } else {
            a[bk].push(ws.merge(ws.isArr(i) ? [] : {}, i));
          }
        });
      } else if (
        b[bk].tagName &&
  ...
  isArr: function(what) {
    return (
      !ws.isNull(what) && what.constructor.toString().indexOf('Array') > -1
    );
  },
  ...
  removeNulls: function(dataSet){
    ...
    dataSet.forEach(function(e){
      ...
      e.forEach(function(v) {
        ...
        if (ws.isStr(v) && Date.parse(v) !== NaN) {
...
(function() {
   ...
   */
  ws.exposeOption = function(option) {};
   ...
   */
  ws.option = function(option, value) {
    if (!ws.isBasic(option)) {
      ws.merge(options, option);
   ...
   */
  ws.options = function(options) {
    Object.keys(options || {}).forEach(function(key) {
      ws.option(key, options[key]);
    });
  };
   ...
   */
  ws.serializeEditorOptions = function() {
    return ws.merge({}, options);
  };
...
* @param {function} fn - the function to call
   ...
   */
  ws.include = function(what, fn, asCSS) {
    var n;

    if (!ws.isStr(what)) {
      return ws.isFn(fn) && fn();
    }

    function next() {
      if (n < what.length - 1) {
        ws.include(what[++n], next);
      }
    ...
    if (includedScripts[what]) {
      ws.log(3, 'script already included, skipping:', what);
      return fn();
    }

    ws.log(3, 'including script', what);
  ...
  ws.getLetterIndex = function (char) {
