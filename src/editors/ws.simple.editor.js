 ...
 *
 *  @example
 *  var simpleEditor = ws.SimpleEditor(document.body, {
 *      availableSettings: [
 *          'title--text',
 *          'subtitle--text'
 *      ]
 ...
 *    > {ws.ChartPreview} - an instance of the chart preview
 *
 ...
 */
ws.SimpleEditor = function(parent, attributes) {
  var properties = ws.merge(
    ...
    events = ws.events(),
    container = ws.dom.cr('div', 'ws-container'),
    expandContainer = ws.dom.cr('div', 'ws-expand-container'),
    mainVSplitter = ws.VSplitter(container, {
      topHeight: '60px',
      noOverflow: true
    }),
    mainToolbar = ws.Toolbar(mainVSplitter.top, {
      additionalCSS: ['ws-header']
    }),
    hsplitter = ws.HSplitter(mainVSplitter.bottom, {
      leftWidth: 30,
      noOverflow: false
    }),
    vsplitterRight = ws.VSplitter(hsplitter.right, {
      noOverflow: true
    }),
    vsplitterLeft = ws.VSplitter(hsplitter.left, {
      topHeight: 60
    }),
    preview = ws.ChartPreview(vsplitterRight.top, {
      defaultChartOptions: properties.defaultChartOptions,
      expandTo: expandContainer
    }),
    importer = ws.SimpleDataPage(vsplitterRight.bottom, vsplitterLeft.bottom, properties.importer, preview, vsplitterRight.top, {}),
    //importer = ws.DataImporter(vsplitterRight.bottom, properties.importer),
    //importer = ws.DataTable(vsplitterRight.bottom),
    //dataPage = ws.SimpleDataPage(),
    customizer = ws.SimpleCustomizer(vsplitterLeft.top, {
      availableSettings: properties.availableSettings
    }),
    cmenu = ws.DefaultContextMenu(preview);

  ///////////////////////////////////////////////////////////////////////////

  properties.features = ws.arrToObj(properties.features.split(' '));
  ...
  /** Force a resize of the editor
     *  @memberof ws.SimpleEditor
     */
  function resize() {
    var ps = ws.dom.size(container);
  ...
  ws.dom.ap(ws.dom.get(parent), container, expandContainer);

  ws.dom.on(window, 'resize', resize);

  ws.dom.ap(
    mainToolbar.left,
    ws.dom.style(ws.dom.cr('div', 'ws-logo'), {
      'background-image':
